...
import { TonClient4, Address, beginCell, WalletContractV4, TupleReader } from 'ton';
...
describe('TokensService', () => {
  ...
  beforeEach(async () => {
    ...
    when(MockConfigService.get('CENTRAL_WALLET_MNEMONIC')).thenReturn('word1 word2 word3 ...');
  ...
  describe('accrueTokens', () => {
    it('should accrue tokens based on trade volume', async () => {
      ...
      verify(MockRedisService.set(dailyTokensKey, '5', 'EX', 24 * 60 * 60)).once();
    ...
    it('should not accrue tokens if daily limit reached', async () => {
      ...
      verify(MockRedisService.get(cacheKey)).once();
      verify(MockRedisService.get(dailyTokensKey)).once();
      verify(MockRedisService.set(anything(), anything(), anything(), anything())).never();
    ...
    it('should accrue partial tokens if close to daily limit', async () => {
      ...
      verify(MockRedisService.set(dailyTokensKey, '10', 'EX', 24 * 60 * 60)).once();
      verify(MockUserRepository.save(mockUser)).once();
  ...
  describe('withdrawTokens', () => {
    ...
    beforeEach(() => {
      ...
      when(MockTonClient.runMethod(anything(), anything(), anything())).thenResolve({
          exitCode: 0,
          result: [{ type: 'slice', cell: beginCell().storeAddress(mockJettonWalletAddress).endCell() }],
          resultRaw: null,
          block: { workchain: 0, shard: '8000000000000000', seqno: 12345, fileHash: 'hash1', rootHash: 'hash2' },
          shardBlock: { workchain: 0, shard: '8000000000000000', seqno: 12345, fileHash: 'hash1', rootHash: 'hash2' },
          reader: new TupleReader([{ type: 'slice', cell: beginCell().storeAddress(mockJettonWalletAddress).endCell() }]),
      });
      ...
      when(mockWallet.createTransfer(anything())).thenReturn(beginCell().endCell());
    ...
    it('should throw BadRequestException for non-positive amount', async () => {
      ...
      await expect(service.withdrawTokens(invalidDto)).rejects.toThrow(BadRequestException);
    ...
    it('should throw NotFoundException if user not found', async () => {
      ...
      await expect(service.withdrawTokens(withdrawDto)).rejects.toThrow(NotFoundException);
      verify(MockUserRepository.findOne({ where: { id: withdrawDto.userId } })).once();
    });
    ...
    it('should throw BadRequestException for insufficient balance', async () => {
      ...
      await expect(service.withdrawTokens(highAmountDto)).rejects.toThrow(BadRequestException);
      verify(MockUserRepository.findOne({ where: { id: withdrawDto.userId } })).once();
    });

    it('should throw BadRequestException for missing TON configuration', async () => {
      when(MockConfigService.get('CENTRAL_WALLET_MNEMONIC')).thenReturn(undefined);
      when(MockUserRepository.findOne({ where: { id: withdrawDto.userId } })).thenResolve(mockUser);

      await expect(service.withdrawTokens(withdrawDto)).rejects.toThrow(BadRequestException);
      verify(MockUserRepository.findOne({ where: { id: withdrawDto.userId } })).once();
    });

    it('should throw BadRequestException on TON transaction failure', async () => {
      when(MockUserRepository.findOne({ where: { id: withdrawDto.userId } })).thenResolve(mockUser);
      when(MockTonClient.sendMessage(anything())).thenReject(new Error('TON error'));
